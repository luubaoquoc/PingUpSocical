...
const Profile = () => {
  ...
  return user ? (
    <div className='relative h-full overflow-y-scroll bg-gray-50 p-6'>
          ...
          <div className='bg-white rounded-xl shadow p-1 flex max-w-md mx-auto'>
            {['posts', 'media', 'likes'].map((tab) => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                className={`flex-1 px-4 py-2 text-sm rounded-lg font-medium transition-colors cursor-pointer
                ...
                <>
                  {post.image_urls.map((img, index) => (
                    <Link key={index} target='_blank' to={img} className='relative group'>
                      <img src={img} alt="" className='w-64 aspect-video object-cover ' />
      ...
      {/* Edit Modal */}
      {showEdit && <p>Show profile edit modal</p>}
    </div>
  ) : (<Loading />)
}
